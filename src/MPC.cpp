...
// N = 10 is a good value that balance between prediction and efficiency
size_t N = 10;
//dt = 0.1 equals to the 100ms latency of the track data, which make the prediction fast and accurate enough
double dt = 0.1;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	if (ok) {
		...
		//add the calculated x and y values for display on the simulator
		for (int i = 0; i < N; i++) {
			x_vals.push_back(solution.x[x_start + i]);
			y_vals.push_back(solution.x[y_start + i]);
		}
		//return the delta and throtle value
